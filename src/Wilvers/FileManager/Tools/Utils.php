...
class Utils {
    /**
     * @TODO evalute utility of each function
     * Camelcase function name
     * remove global call to vars , session and request
     * convert each call to function with class call self::
     * split in different class
     * - img
     * - file and folder
     * - string
     * - ...
     ...
     * @param type $headers
     * @return \FileManager\Tools\Response
     */
    public static function response($content = '', $statusCode = 200, $headers = array()) {
        return new \FileManager\Tools\Response($content, $statusCode, $headers);
    }
     ...
     * @return  bool
     */
    public static function duplicate_file($old_path, $name) {
     ...
     * @return bool
     */
    public static function rename_file($old_path, $name, $transliteration) {
        $name = self::fix_filename($name, $transliteration);
        if (file_exists($old_path)) {
            $info = pathinfo($old_path);
            $new_path = $info['dirname'] . "/" . $name . "." . $info['extension'];
            if (file_exists($new_path) && $old_path == $new_path) {
     ...
     * @return bool
     */
    public static function rename_folder($old_path, $name, $transliteration) {
        $name = self::fix_filename($name, $transliteration, false, '_', true);
        if (file_exists($old_path)) {
     ...
     * @throws \Exception
     */
    public static function create_img($imgfile, $imgthumb, $newwidth, $newheight = null, $option = "crop") {
        $result = false;
     ...
     */
    public static function create_folder($path = null, $path_thumbs = null) {
     ...
     * @param  string  $ext
     */
    public static function check_files_extensions_on_phar($phar, &$files, $basepath, $ext) {
        foreach ($phar as $file) {
            if ($file->isFile()) {
                ...
                if ($file->isDir()) {
                    ...
                    self::check_files_extensions_on_phar($iterator, $files, $basepath . $file->getFileName() . '/', $ext);
                }
            }
        }
    }
     ...
     * @return string
     */
    public static function fix_filename($str, $transliteration, $convert_spaces = false, $replace_with = "_", $is_folder = false) {
        if ($convert_spaces) {
...
// So we add that default 'file' name to fix that issue.
    ...
    public static function fix_path($path, $transliteration, $convert_spaces = false, $replace_with = "_") {
        $info = pathinfo($path);
        $tmp_path = $info['dirname'];
        $str = self::fix_filename($info['filename'], $transliteration, $convert_spaces, $replace_with);
        if ($tmp_path != "") {
     ...
     * @return bool
     */
    public static function new_thumbnails_creation($targetPath, $targetFile, $name, $current_path, $relative_image_creation, $relative_path_from_current_pos, $relative_image_creation_name_to_prepend, $relative_image_creation_name_to_append, $relative_image_creation_width, $relative_image_creation_height, $relative_image_creation_option, $fixed_image_creation, $fixed_path_from_filemanager, $fixed_image_creation_name_to_prepend, $fixed_image_creation_to_append, $fixed_image_creation_width, $fixed_image_creation_height, $fixed_image_creation_option) {
        ...
        if ($relative_image_creation) {
            foreach ($relative_path_from_current_pos as $k => $path) {
                ...
                if (!self::endsWith($targetPath, $path)) {
                    if (!self::create_img($targetFile, $targetPath . $path . $relative_image_creation_name_to_prepend[$k] . $info['filename'] . $relative_image_creation_name_to_append[$k] . "." . $info['extension'], $relative_image_creation_width[$k], $relative_image_creation_height[$k], $relative_image_creation_option[$k])) {
                        $all_ok = false;
                    }
                }
            }
...
//create fixed thumbs
        if ($fixed_image_creation) {
            foreach ($fixed_path_from_filemanager as $k => $path) {
                ...
                $info = pathinfo($name);
                if (!self::create_img($targetFile, $base_dir . $fixed_image_creation_name_to_prepend[$k] . $info['filename'] . $fixed_image_creation_to_append[$k] . "." . $info['extension'], $fixed_image_creation_width[$k], $fixed_image_creation_height[$k], $fixed_image_creation_option[$k])) {
                    $all_ok = false;
                }
            }
        }
...
// After more feedback will merge it with rrename
     ...
     * @return  bool
     */
    public static function chmod_logic_helper($perm, $val) {
        $valid = array(
            1 => array(1, 3, 5, 7),
            2 => array(2, 3, 6, 7),
            4 => array(4, 5, 6, 7)
